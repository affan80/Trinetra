...
from backend.app.db.models import CollectionProvenance, MonitoringProfile, RawObject
...
def preserve(db, job, source, result, *, object_store, collector_version="layer3-v0.1"):
    ...
    digest = sha256(value["body"])
    uri = object_store.put(value["body"], digest)
    profile = db.get(MonitoringProfile, job.profile_id)
    ...
    evidence = RawObject(
        ...
        source_url=value["requested_url"],
        ...
        last_modified=value["headers"].get("last-modified"),
    ...
    provenance = CollectionProvenance(
        raw_object_id=evidence.id,
        job_id=job.id,
        source_id=source.id,
        collector_id="trinetra-layer3",
        collector_version=collector_version,
        adapter_type="RSS",
        requested_url=value["requested_url"],
        final_url=value["final_url"],
        redirect_chain=value["redirect_chain"],
        http_headers_sanitized=value["headers"],
