...
class ObjectStore:
    """Content-addressed storage; local volume by default, optional S3 endpoint."""

    def __init__(self, root: str | None = None):
        self.root = Path(root or os.getenv("RAW_OBJECT_ROOT", "/tmp/trinetra-raw-evidence"))
        self.endpoint = None if root else os.getenv("MINIO_ENDPOINT")
        self.client = None
        if self.endpoint:
            from minio import Minio
            self.client = Minio(
                self.endpoint,
                access_key=os.getenv("MINIO_ACCESS_KEY", "trinetra"),
    ...
    def put(self, data: bytes, sha: str | None = None) -> str:
        ...
        if self.client:
            ...
            key = f"raw/{sha[:2]}/{sha}"
            self.client.put_object(bucket, key, BytesIO(data), len(data), content_type="application/octet-stream")
