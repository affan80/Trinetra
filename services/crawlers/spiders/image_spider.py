...
class ImageSpider(scrapy.Spider):
    ...
    def __init__(self, urls=None, domains=None, max_pages=100, *args, **kwargs):
    ...
    def parse(self, response):
        ...
        for image in scraper.extract_images():
            if image["url"] not in self.seen_images:
                ...
                img_count += 1
                yield self.build_item(response, domain, page_title, image)
    ...
    def build_item(self, response, domain, page_title, image):
        return ImageItem(
            source_name=domain,
            source_type="image",
